...
def create_simplified_tessellation(label, num_vertices=30, initial_step_size=2):
    # label: 3D numpy array label map (binary values)
    # num_vertices: number of vertices (default = 30)
    # initial_step_size: step size for the marching cubes algorithm
...
# In[]: Draw triangles on both the 3d and 2d images to find correspondence
    ...
    for face_2d in tqdm(faces_2d[:]):
        ...
        if triangle_area(coord_3d) > 1e-10:
            ...
            for ratio in [0.96, 0.97, 0.98]:
                ...
                triangles_2d[
                    points[0, :].astype(int), points[1, :].astype(int)
                ] = colour
